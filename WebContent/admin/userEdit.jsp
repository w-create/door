					...
					<div class="input-medium help-validate">
						<div class="input-group">
							<input id="_account" name="account" type="text" value="${user.account}" class="form-data"
								ajax="{url : '${_path}/tscp?action=UserAction_validateAccount', dataId : '_editForm'}"
		...
		jQuery(function($) {
			...
			$("#_submit").click(function(){
				if($('#_editForm').validate()){
					...
					var btn = $(this);
					btn.button('loading');
					$.post("${_path}/tscp?action=UserAction_save", $.formJson('_editForm'),function(d) {
						if(d){
							btn.button('reset');
							if(d.code == 1){
								$.aceRedirect("${_path}/tscp?action=UserAction_user");
							}
							else {
