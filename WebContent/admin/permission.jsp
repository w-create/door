		...
		jQuery(function($) {
    		var setting = {
	            ...
	            async: {
					...
					dataType: "text",
					url: "${_path}/tscp?action=PermissionAction_nodes"
				},
	            check: {
	                enable: false
	            },
			...
			$("#_submit").click(function(){
				if($('#_editForm').validate()){
					var btn = $(this);
					btn.button('loading');
					$.post("${_path}/tscp?action=PermissionAction_save", $.formJson('_editForm'),function(d) {
						if(d){
							btn.button('reset');
							$.gritter.add({
								text: d.message,
	        ...
	        function addDiyDom(treeId, treeNode) {
				...
				var editStr = "&nbsp;<a id='add_" + treeNode.id + "' class='blue' href='javascript:void(0)' title='添加'>		<i class='ace-icon fa fa-plus bigger-110'></i>	</a>";
	       		if(treeNode.id) {
					editStr += "&nbsp;<a id='edit_" + treeNode.id + "' class='green' permission='/tscp?action=PermissionAction_save' href='javascript:void(0)' title='修改'>		<i class='ace-icon fa fa-pencil bigger-110'></i>	</a>";
					editStr += "&nbsp;<a id='delete_" + treeNode.id + "' class='red' permission='/tscp?action=PermissionAction_delete' href='javascript:void(0)' title='删除'>		<i class='ace-icon fa fa-trash-o bigger-110'></i>	</a>";
				}
				aObj.after(editStr);
				...
				if(deleteBtn){
					deleteBtn.bind("click", function(){
						bootbox.confirm( "会删除该权限所有子权限，确认要删除吗?",function(result) {
							if(result) {
								$.post("${_path}/tscp?action=PermissionAction_delete", {"id":treeNode.id, "appId":$("#_searchAppId option:selected").val()},function(d) {
									if(d){
										if(d.code == 1){
											$.gritter.add({
												text: d.message,
