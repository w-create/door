				...
				<div class="col-sm-9">
					<div class="clearfix help-validate">
						<input id="_code" name="code" type="text" value="${app.code}" class="form-data col-xs-10 col-sm-5" placeholder="应用编码"
							required="true" ajax="{url : '${_path}/tscp?action=AppAction_validateCode', type : 'post', dataId : '_editForm'}" maxlength="64"/>
					</div>
				</div>
		...
		jQuery(function($) {
			...
			$("#_submit").click(function(){
				if($('#_editForm').validate()){
					var btn = $(this);
					btn.button('loading');
					$.post("${_path}/tscp?action=AppAction_save", $.formJson('_editForm'),function(d) {
						if(d){
							btn.button('reset');
							if(d.code == 1){
								$.aceRedirect("${_path}/tscp?action=AppAction_app");
							}
							else {
